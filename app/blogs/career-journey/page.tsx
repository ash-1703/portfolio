...
async function getCareerJourneyBlogs(): Promise<BlogListItem[]> {
  return await client.fetch(`
    *[_type == "blogPost" && category == "career-journey"] | order(date desc) {
      "slug": slug.current,
      date,
      title,
      excerpt,
...
export default async function CareerJourneyBlogsPage() {
  const blogs = await getCareerJourneyBlogs();
      ...
      <section className="mx-auto max-w-4xl px-4 pb-12">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-4xl md:text-5xl font-bold">
            Career & Learning <span className="gradient-text">Journey</span>
          </h1>
        </div>
        <p className="text-lg text-slate-200 max-w-3xl">
          Personal reflections on transitioning from technology to healthcare, 
          learning in public, and building interdisciplinary expertise.
        </p>
      </section>
      ...
      <section className="mx-auto max-w-4xl px-4 pb-20">
        <div className="space-y-6">
          {blogs.map((blog: BlogListItem, idx: number) => (
            <Link
              key={idx}
              href={`/blogs/career-journey/${blog.slug}`}
